---
lang: "it"
translationKey: "chirurgia-mtf"
title: "Chirurgia di affermazione di genere MTF: guida completa"
seoTitle: "Tutti gli interventi chirurgici per donne trans (MTF): guida completa"
slug: "chirurgia-mtf"
description: "Chirurgia MTF in Italia: vaginoplastica, orchiectomia, femminilizzazione viso (FFS), mammoplastica additiva. Costi, centri, tempi di recupero e soddisfazione."
category: "scienza"
tags: ["chirurgia", "donne trans", "MTF", "vaginoplastica", "mammoplastica", "FFS", "chirurgia vocale", "orchiectomia", "transizione", "mastoplastica additiva", "Italia", "tipi intervento"]
date: "2026-02-21"
updated: "2026-02-21"
image: "/images/wiki/chirurgia-mtf.webp"
sources:
  - title: "Standards of Care for the Health of Transgender and Gender Diverse People, Version 8 (WPATH SOC-8)"
    url: "https://www.tandfonline.com/doi/full/10.1080/26895269.2022.2100644"
    year: 2022
  - title: "Complications and satisfaction in transwomen receiving breast augmentation: short- and long-term outcomes"
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC9166844/"
    year: 2022
  - title: "Prospective Quality-of-Life Outcomes after Facial Feminization Surgery: An International Multicenter Study"
    url: "https://pubmed.ncbi.nlm.nih.gov/32459779/"
    year: 2020
  - title: "Facial Feminization Surgery: A Systematic Review of Perioperative Surgical Planning and Outcomes"
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC8929523/"
    year: 2022
  - title: "Long-term Voice Outcomes After Wendler Glottoplasty: A Systematic Review and Meta-analysis"
    url: "https://pubmed.ncbi.nlm.nih.gov/37730332/"
    year: 2023
  - title: "Chondrolaryngoplasty Under Local Anesthesia in Transgender Women: Technique and Outcomes"
    url: "https://pubmed.ncbi.nlm.nih.gov/31045091/"
    year: 2019
  - title: "Breast augmentation in transgender women: Technique and outcomes"
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC8419584/"
    year: 2021
  - title: "Orchiectomy as a Bridge or Alternative to Vaginoplasty"
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC8261310/"
    year: 2021
  - title: "Corte Costituzionale, Sentenza n. 143/2024 - Autorizzazione interventi chirurgici di affermazione di genere"
    url: "https://www.cortecostituzionale.it/scheda-pronuncia/2024/143"
    year: 2024
  - title: "Il trattamento chirurgico di affermazione di genere - Infotrans"
    url: "https://www.infotrans.it/it-schede-35-chirurgia_transgender"
    year: 2024
  - title: "Regret after Gender-affirmation Surgery: A Systematic Review and Meta-analysis of Prevalence"
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC8099405/"
    year: 2021
  - title: "Complications and Patient-reported Outcomes in Transfemale Vaginoplasty: An Updated Systematic Review and Meta-analysis"
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC7984836/"
    year: 2021
  - title: "Electrolysis and laser for genital area hair removal prior to gender-affirming vaginoplasty: A systematic review"
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC9537259/"
    year: 2022
  - title: "Gender-Affirming Surgical History, Satisfaction, and Unmet Needs Among Transgender Adults"
    url: "https://jamanetwork.com/journals/jamanetworkopen/fullarticle/2839065"
    year: 2024
faq:
  - question: "Quali interventi chirurgici esistono per le donne trans?"
    answer: "Le principali opzioni chirurgiche per le donne trans comprendono: vaginoplastica (creazione della neovagina), mammoplastica additiva (aumento del seno), femminilizzazione facciale (FFS), riduzione del pomo d'Adamo, chirurgia vocale, orchiectomia e procedure di body contouring. Non tutte le donne trans scelgono la chirurgia, e chi la sceglie seleziona gli interventi in base alle proprie necessita."
  - question: "Quanto costano gli interventi MTF in Italia?"
    answer: "I costi variano molto. Tramite SSN, gli interventi principali (vaginoplastica, mammoplastica additiva, orchiectomia) sono coperti ma con liste di attesa di 2-5 anni. Privatamente, i costi indicativi sono: vaginoplastica 15.000-25.000 euro, mammoplastica 4.000-8.000 euro, FFS 8.000-30.000 euro a seconda delle procedure, orchiectomia 2.000-4.000 euro, chirurgia vocale 3.000-8.000 euro."
  - question: "In che ordine si fanno gli interventi MTF?"
    answer: "Non esiste un ordine obbligatorio. Una sequenza comune prevede: terapia ormonale per almeno 12 mesi, poi eventualmente mammoplastica additiva e orchiectomia, quindi vaginoplastica. La FFS e la chirurgia vocale possono essere eseguite in qualsiasi momento. La sequenza dipende dalle priorita individuali, dalla disponibilita dei centri e dalle condizioni cliniche."
  - question: "La chirurgia MTF e coperta dal Servizio Sanitario Nazionale?"
    answer: "Il SSN copre vaginoplastica, mammoplastica additiva e orchiectomia nei centri pubblici specializzati. La FFS, la chirurgia vocale e il body contouring non sono generalmente coperti. Le liste di attesa SSN possono essere molto lunghe, da 2 a 5 anni."
  - question: "Serve ancora l'autorizzazione del tribunale per operarsi?"
    answer: "No. Dal 2024, con la sentenza n. 143 della Corte Costituzionale, non e piu necessaria l'autorizzazione del tribunale per gli interventi chirurgici di affermazione di genere. La decisione e ora esclusivamente medica, tra la paziente e il team sanitario."
related:
  - chirurgia-affermazione-di-genere
  - vaginoplastica-come-funziona
  - transizione-medica-cosa-cambia
  - corpo-cambia-dopo-ormoni
  - legge-164-italia
---

La transizione chirurgica per le donne trans non si riduce a un singolo intervento. Esiste un ventaglio di procedure, ciascuna con indicazioni, tempistiche e implicazioni diverse, che le donne trans possono scegliere in base alle proprie esigenze. Alcune ne scelgono una sola, altre combinano più interventi, altre ancora nessuno. Questo articolo presenta una panoramica completa di tutte le opzioni chirurgiche disponibili per le donne trans (MTF), con dati su tecniche, tempi di recupero, costi in Italia, centri di riferimento e cosa dice la legge italiana aggiornata. Per una panoramica generale che include anche gli interventi per uomini trans, rimandiamo all'articolo sulla [chirurgia di affermazione di genere](/wiki/chirurgia-affermazione-di-genere). Non è un consiglio medico: è un punto di partenza informativo, basato sulla letteratura scientifica e sulle linee guida internazionali.

## Prerequisiti e criteri di accesso

Prima di qualsiasi intervento chirurgico di affermazione di genere, esistono criteri clinici definiti dagli **Standards of Care versione 8 (SOC-8)** della World Professional Association for Transgender Health (WPATH), pubblicati nel 2022 [1]. Questi criteri non sono barriere arbitrarie ma strumenti per garantire che la persona sia adeguatamente informata e preparata.

I requisiti generali per la chirurgia di affermazione di genere secondo i SOC-8 includono:

- **Incongruenza di genere persistente e ben documentata**
- **Capacità di fornire consenso informato** per lo specifico intervento
- **Raggiungimento dell'età adulta** (o requisiti specifici per minori, che si applicano solo alla top surgery e non alla bottom surgery)
- **Eventuali condizioni di salute mentale o fisica ragionevolmente ben controllate**

Per interventi specifici si aggiungono criteri ulteriori [1]:

- **Vaginoplastica e orchiectomia**: almeno 12 mesi di terapia ormonale (salvo controindicazioni o scelta informata diversa) e almeno 6 mesi di vita nel ruolo di genere congruente con la propria identità. La terapia ormonale è raccomandata per garantire la maturazione dei tessuti necessari all'intervento.
- **Mammoplastica additiva**: almeno 12 mesi di terapia ormonale per permettere il massimo sviluppo naturale del seno prima di valutare l'intervento.
- **FFS, chirurgia vocale, condrolaringoplastica**: non richiedono periodi obbligatori di terapia ormonale, anche se molte pazienti iniziano gli ormoni prima di accedere a queste procedure.

In Italia, il percorso prevede una **valutazione psicologica** di almeno sei mesi secondo il protocollo ONIG (Osservatorio Nazionale sull'Identità di Genere), con l'obiettivo di esplorare l'incongruenza di genere, preparare la persona alle scelte successive e documentare il percorso clinico [10]. Questo non è un giudizio sulla legittimità dell'identità della persona ma un processo di accompagnamento clinico.

## Il quadro legale in Italia

### La sentenza 143/2024 della Corte Costituzionale

Il panorama legale italiano ha subito un cambiamento significativo nel 2024. La Corte Costituzionale, con la **sentenza n. 143/2024**, ha dichiarato l'illegittimità costituzionale dell'obbligo di ottenere l'autorizzazione del tribunale per sottoporsi a interventi chirurgici di adeguamento dei caratteri sessuali [9]. Fino a quel momento, chi desiderava operarsi doveva prima ottenere il via libera di un giudice, un requisito che non esisteva per nessun altro tipo di intervento chirurgico.

Dopo la sentenza, la chirurgia di affermazione di genere è diventata una decisione medica tra la paziente e il team sanitario, esattamente come qualsiasi altro intervento. Resta ovviamente necessario il percorso clinico previsto dalle linee guida.

Questo si affianca a quanto già stabilito dalla Cassazione nel 2015: la [rettifica anagrafica](/wiki/legge-164-italia) non richiede interventi chirurgici. In altre parole, la chirurgia è una scelta libera, né imposta né condizionante per il riconoscimento legale della propria identità.

### Copertura del Servizio Sanitario Nazionale

Il SSN copre le principali procedure di affermazione di genere nei centri pubblici specializzati: **vaginoplastica, mammoplastica additiva e orchiectomia** sono interventi erogabili [10]. La copertura è reale ma il problema principale sono le **liste di attesa**: i centri SSN eseguono un numero limitato di interventi all'anno e i tempi di attesa vanno da 2 a 5 anni.

**Non sono generalmente coperti** dal SSN: la femminilizzazione facciale (FFS), la chirurgia vocale, la condrolaringoplastica e le procedure di body contouring [10]. Questi interventi restano a carico della paziente, salvo rare eccezioni in specifiche regioni o situazioni cliniche particolari.

## Vaginoplastica

La vaginoplastica è l'intervento più noto e spesso considerato il più significativo nel percorso chirurgico delle donne trans. Consiste nella creazione di una neovagina funzionale, con vulva, grandi e piccole labbra, clitoride e canale vaginale. È un intervento complesso, con decenni di perfezionamento tecnico e una letteratura scientifica ampia.

Le tecniche principali sono:

- **Inversione del lembo penile-scrotale**: la più diffusa a livello mondiale. Utilizza la pelle del pene e dello scroto per creare il canale vaginale e la vulva. Il glande viene rimodellato per formare il neoclitoride, preservando l'innervazione e la sensibilità.
- **Colonvaginoplastica (sigmoidovaginoplastica)**: utilizza un segmento del colon sigmoideo per il canale vaginale. Indicata quando il tessuto genitale è insufficiente o come revisione.
- **Vaginoplastica peritoneale**: tecnica più recente che utilizza il peritoneo pelvico. Offre il vantaggio di una certa autolubrificazione.

I dati sulla soddisfazione sono solidi [12]: il 91-93% delle pazienti riporta soddisfazione complessiva, il 90% soddisfazione estetica, il 70-80% la capacità di raggiungere l'orgasmo. Il tasso di rimpianto è circa il 2%.

Il recupero richiede **6-8 settimane di riposo** e un programma di **dilatazione vaginale** regolare, indispensabile nei primi mesi e che continua con frequenza decrescente nel tempo. La guarigione completa si raggiunge in circa 12 mesi.

**Costi**: tramite SSN l'intervento è coperto (liste di attesa 2-5 anni) [10]. Privatamente, in Italia i costi oscillano tra **15.000 e 25.000 euro** a seconda del chirurgo e della struttura. All'estero, centri come il Suporn Clinic in Thailandia o il Ghent University Hospital in Belgio sono tra i più noti, con costi variabili.

Per un'analisi approfondita di tecniche, preparazione, rischi e recupero, rimandiamo all'articolo dedicato sulla [vaginoplastica](/wiki/vaginoplastica-come-funziona).

## Mammoplastica additiva (aumento del seno)

### Quando serve e quando no

La terapia estrogenica induce uno [sviluppo mammario naturale](/wiki/corpo-cambia-dopo-ormoni), ma i risultati variano molto da persona a persona. In media, dopo 2-3 anni di terapia ormonale, lo sviluppo raggiunge una taglia A-B, spesso inferiore alle aspettative [7]. Uno studio prospettico multicentrico ha mostrato che dopo un anno di ormoni il volume mammario medio raggiunge circa 150 ml, equivalente a poco più di una coppa A. Lo sviluppo può continuare fino a 3-5 anni dall'inizio della terapia.

Per questo motivo, le linee guida WPATH SOC-8 raccomandano di attendere **almeno 12 mesi di terapia ormonale** prima di considerare l'intervento di aumento del seno, per permettere il massimo sviluppo naturale [1]. Alcune pazienti raggiungono risultati soddisfacenti solo con gli ormoni e non desiderano la chirurgia. Per altre, la mammoplastica additiva è necessaria per ridurre la disforia legata al torace.

### Tecnica chirurgica

La mammoplastica additiva nelle donne trans segue le stesse tecniche utilizzate per le donne cisgender, con alcune considerazioni specifiche [7]:

- **Tipo di protesi**: le protesi in silicone coesivo sono le più utilizzate. Le protesi rotonde sono preferite rispetto a quelle anatomiche (a goccia) nelle donne trans, perché la base toracica è in genere più ampia e il tessuto mammario nativo è minore, rendendo le protesi rotonde esteticamente più naturali in questo contesto.
- **Posizionamento**: la posizione **sottomuscolare** (sotto il muscolo grande pettorale) o **dual plane** è generalmente preferita nelle donne trans. Il muscolo offre una copertura aggiuntiva della protesi, fondamentale quando il tessuto mammario è scarso, riducendo la visibilità dei bordi dell'impianto.
- **Accesso chirurgico**: l'incisione inframammaria (nel solco sotto il seno) è la più comune.

### Recupero e risultati

Il recupero dalla mammoplastica additiva prevede:

- **1-2 settimane** di riposo domiciliare
- **3-4 settimane** prima di riprendere attività fisiche leggere
- **6-8 settimane** prima di riprendere attività intense
- **3-6 mesi** per l'assestamento definitivo delle protesi

La soddisfazione post-operatoria è elevata: circa il **75-80%** delle pazienti riporta punteggi alti di soddisfazione [2]. Le complicanze sono comparabili a quelle delle mammoplastiche in donne cisgender: il tasso di contrattura capsulare (indurimento della capsula fibrosa attorno alla protesi) è circa il 3-5%, le re-operazioni per complicanze intorno al 5-8% [2].

### Costi

- **SSN**: coperto nei centri pubblici specializzati, ma con liste di attesa lunghe [10].
- **Privato in Italia**: da **4.000 a 8.000 euro** a seconda della struttura, del tipo di protesi e del chirurgo.
- **Privato in Europa**: prezzi comparabili nella maggior parte dei paesi UE. La Spagna e il Belgio offrono centri specializzati con esperienza specifica su pazienti trans.

## Femminilizzazione facciale (FFS)

### Perché il viso conta

La Facial Feminization Surgery (FFS) è un insieme di procedure chirurgiche che modificano le caratteristiche ossee e dei tessuti molli del viso per attenuare i tratti percepiti come maschili [4]. Il viso è il principale marcatore di genere nelle interazioni sociali quotidiane: spesso è il fattore che più influenza il passing e la percezione sociale. Per questo motivo, molte donne trans considerano la FFS altrettanto o più importante della bottom surgery per il proprio benessere quotidiano.

Uno studio multicentrico prospettico pubblicato nel 2020 ha mostrato che il punteggio mediano di femminilizzazione facciale sale da 47,2 prima dell'intervento a **80,6 dopo sei o più mesi**, con miglioramenti significativi nella qualità della vita e riduzione della disforia [3]. La soddisfazione è valutata come eccellente dalla maggioranza delle pazienti.

### Le procedure

La FFS comprende diverse procedure, spesso combinate in una o due sessioni chirurgiche [4]:

**Rimodellamento della fronte (frontal bone contouring)**
È la procedura più richiesta e spesso la più impattante. L'osso frontale maschile presenta una prominenza (bossing) sopra le orbite che viene ridotta. Esistono tre approcci:
- Tipo I: limatura dell'osso (quando il bossing è lieve)
- Tipo III: osteotomia con riposizionamento della parete anteriore del seno frontale (quando il bossing è pronunciato). È la tecnica più efficace e più utilizzata.
La cicatrice è nascosta lungo l'attaccatura dei capelli o nel cuoio capelluto.

**Rinoplastica (rimodellamento del naso)**
La rinoplastica di femminilizzazione mira a creare un naso più piccolo, con dorso più stretto, punta più definita e lievemente ruotata verso l'alto. Le tecniche sono le stesse della rinoplastica estetica convenzionale, adattate agli obiettivi specifici.

**Rimodellamento della mascella e del mento (jaw and chin contouring)**
La mascella maschile tende ad avere angoli più pronunciati e un mento più largo e sporgente. Il rimodellamento prevede la riduzione degli angoli mandibolari (rasping o osteotomia) e la genioplastica (riduzione e rimodellamento del mento). L'accesso è generalmente intraorale, senza cicatrici visibili.

**Lifting del labbro superiore (lip lift)**
Accorcia la distanza tra il naso e il labbro superiore, un tratto sessualmente dimorfico. La cicatrice è alla base del naso e diventa quasi invisibile col tempo.

**Avanzamento del cuoio capelluto e correzione dell'attaccatura**
Corregge la recessione temporale tipica della linea dei capelli maschile, abbassando l'attaccatura e arrotondando gli angoli.

**Blefaroplastica e lifting sopracciliare**
Modificano la posizione e la forma delle sopracciglia e delle palpebre per un aspetto più femminile.

### Recupero

I tempi di recupero variano in base al numero di procedure combinate:

- **Singola procedura** (es. solo rinoplastica): **2-3 settimane** per la ripresa delle attività normali
- **FFS combinata** (fronte + naso + mascella): **3-6 settimane** per il gonfiore iniziale, **3-6 mesi** per la risoluzione completa del gonfiore e l'assestamento dei tessuti
- Il risultato definitivo è visibile dopo **6-12 mesi**

Le complicanze sono generalmente moderate: gonfiore prolungato, alterazioni temporanee della sensibilità, asimmetrie lievi. Complicanze serie (lesioni nervose permanenti, infezioni ossee) sono rare nei centri specializzati, con tassi inferiori al 2% [4].

### Costi e centri

La FFS rappresenta uno degli investimenti economici più significativi nel percorso di transizione perché **non è coperta dal SSN** nella quasi totalità dei casi [10].

- **Italia (privato)**: da **8.000 a 30.000 euro** a seconda del numero e della complessità delle procedure. Una FFS completa (fronte, naso, mascella, mento) si colloca tipicamente nella fascia 15.000-25.000 euro.
- **Spagna**: il Team Facial (Marbella) e il 2pass Clinic (non più attivo) erano storicamente tra i centri di riferimento europei. Attualmente, chirurghi come il Dr. Luis Capitán operano a costi simili a quelli italiani.
- **Belgio**: il Ghent University Hospital offre FFS con chirurghi esperti.
- **Argentina e Thailandia**: destinazioni frequentate per costi più accessibili, con chirurghi di fama internazionale.

I centri italiani con esperienza in FFS includono chirurghi privati a Milano, Roma e Torino, ma la specializzazione italiana in questo ambito è meno consolidata rispetto alla Spagna e al Belgio.

## Riduzione del pomo d'Adamo (condrolaringoplastica)

### Tecnica

La condrolaringoplastica, comunemente chiamata "tracheal shave", è un intervento relativamente semplice che riduce la prominenza della cartilagine tiroidea (il pomo d'Adamo) [6]. La cartilagine tiroidea è più prominente negli individui con sviluppo puberale maschile, e la sua riduzione è uno degli interventi con il rapporto efficacia/invasività più favorevole.

La procedura prevede:
- Un'incisione di 2-3 cm nella regione cervicale, posizionata in una piega cutanea naturale per minimizzare la visibilità della cicatrice
- La rasatura (shaving) della cartilagine tiroidea fino al profilo desiderato
- La sutura, spesso con punti riassorbibili

L'intervento può essere eseguito in **anestesia locale** con sedazione o in anestesia generale e dura circa 30-45 minuti [6].

### Recupero

- **1-2 settimane** di riposo
- Gonfiore e lividi locali che si risolvono in **7-14 giorni**
- Ritorno alle attività normali dopo **1 settimana**
- Risultato definitivo visibile dopo **1-2 mesi**

L'intervento **non influisce sulla voce** se eseguito correttamente, poiché non modifica le corde vocali [6]. Il chirurgo deve fare attenzione a non rasare eccessivamente la cartilagine nella zona dove si inseriscono i legamenti vocali.

### Rischi

I rischi sono minimi: infezione (rara), cicatrice visibile (dipende dalla tecnica e dalla predisposizione individuale), alterazione temporanea della voce (transitoria e rara). In mani esperte, le complicanze sono estremamente rare [6].

### Costi

- **SSN**: generalmente non coperto.
- **Privato in Italia**: da **1.500 a 3.500 euro**.
- Può essere combinato con la FFS o con la chirurgia vocale nella stessa sessione.

## Chirurgia vocale (glottoplastica)

### Il problema della voce

La [terapia ormonale](/wiki/transizione-medica-cosa-cambia) con estrogeni non modifica la voce nelle donne trans. A differenza del testosterone, che abbassa permanentemente la voce, gli estrogeni non invertono l'effetto dell'allungamento delle corde vocali avvenuto durante la pubertà maschile. La voce resta quindi uno dei marcatori di genere più persistenti e, per molte donne trans, una fonte significativa di disforia.

Le opzioni sono due: il **voice training** (logopedia vocale) e la **chirurgia vocale**. Non si escludono a vicenda.

### Voice training: l'alternativa non chirurgica

Il voice training con un logopedista specializzato è il primo approccio raccomandato. Lavora non solo sulla frequenza fondamentale (pitch) ma anche sulla risonanza, l'intonazione, il ritmo e il pattern linguistico. Molte donne trans ottengono risultati eccellenti con il solo voice training, raggiungendo una voce percepita come femminile senza necessità di chirurgia.

I vantaggi: nessun rischio chirurgico, risultati naturali, controllo completo sulla propria voce. Gli svantaggi: richiede costanza (mesi o anni di pratica), i risultati dipendono dall'impegno individuale, e alcune persone non riescono a raggiungere una frequenza sufficiente con il solo allenamento.

### Glottoplastica di Wendler

La tecnica chirurgica più diffusa è la **glottoplastica di Wendler** (Wendler glottoplasty), un intervento endoscopico che accorcia la porzione vibrante delle corde vocali [5]. In pratica:

- Si accede attraverso la bocca (nessuna incisione esterna)
- Il terzo anteriore delle corde vocali viene de-epitelizzato (rimossa la mucosa superficiale)
- Le due corde vengono unite con suture nel terzo anteriore
- La porzione vibrante si riduce, alzando la frequenza fondamentale della voce

La procedura dura circa 30-60 minuti e può essere eseguita in day surgery.

### Recupero

- **1 settimana di silenzio vocale assoluto**: fondamentale per la guarigione delle suture
- **2-4 settimane** di uso vocale limitato e progressivo
- **3-6 mesi** per la stabilizzazione completa della voce
- Sessioni di logopedia post-operatoria raccomandate per ottimizzare i risultati

### Risultati e rischi

Una revisione sistematica e meta-analisi del 2023 sui risultati a lungo termine della glottoplastica di Wendler ha mostrato [5]:

- Innalzamento medio della frequenza fondamentale di circa **50-80 Hz**
- Frequenza post-operatoria media nella fascia **180-220 Hz** (la frequenza femminile tipica è 165-255 Hz)
- Soddisfazione complessiva elevata nella maggioranza delle pazienti

I rischi specifici includono:
- **Riduzione dell'estensione vocale**: la voce diventa meno flessibile, con perdita delle note più basse e talvolta anche di parte delle note alte
- **Voce "stretta" o tesa**: in alcuni casi il risultato è una voce percepita come forzata
- **Necessità di revisione**: una percentuale delle pazienti richiede un secondo intervento
- **Rottura delle suture**: se il silenzio vocale non viene rispettato, le suture possono cedere e il risultato viene compromesso

Per chi lavora con la voce (cantanti, attrici, insegnanti) la chirurgia vocale va valutata con estrema cautela.

### Costi

- **SSN**: generalmente non coperto.
- **Privato in Italia**: da **3.000 a 8.000 euro** a seconda della struttura e del chirurgo.
- **Corea del Sud**: centro di riferimento mondiale per la chirurgia vocale trans (Yeson Voice Center), con costi intorno ai 5.000-7.000 euro più le spese di viaggio.
- In Europa, chirurghi specializzati operano in Belgio, Paesi Bassi e Germania.

## Orchiectomia

### Quando e perché

L'orchiectomia è la rimozione chirurgica dei testicoli. Per le donne trans può essere scelta come [8]:

- **Procedura autonoma**: per eliminare la produzione endogena di testosterone senza sottoporsi alla vaginoplastica. Dopo l'orchiectomia, la dose di antiandrogeni può essere ridotta o eliminata, semplificando la terapia ormonale e riducendone gli effetti collaterali.
- **Procedura ponte**: come passo intermedio in attesa della vaginoplastica, per chi ha lunghi tempi di attesa e vuole eliminare la necessità di antiandrogeni.
- **Scelta definitiva**: per donne trans che non desiderano la vaginoplastica ma vogliono eliminare la fonte di testosterone.

### Tecnica

L'intervento è relativamente semplice:
- Può essere eseguito in **anestesia locale** con sedazione o in anestesia generale
- Si pratica un'incisione sullo scroto o nella regione inguinale
- I testicoli e i funicoli spermatici vengono rimossi
- La procedura dura circa **30-60 minuti**

### Recupero

- **1-2 settimane** di riposo
- **2-4 settimane** per il recupero completo
- Ritorno alle attività sedentarie dopo **pochi giorni**
- Attività fisiche intense dopo **3-4 settimane**

### Considerazioni importanti

L'orchiectomia è **irreversibile** e comporta la perdita definitiva della fertilità. Chi desidera preservare la possibilità di avere figli biologici deve procedere alla **crioconservazione del seme** prima dell'intervento.

Se si prevede di fare la vaginoplastica in futuro, è importante discutere con il chirurgo la tecnica di orchiectomia: alcune tecniche preservano la pelle scrotale (necessaria per la vaginoplastica), altre no [8]. La **tecnica inguinale semplice** è generalmente preferita per non compromettere il tessuto scrotale.

### Costi

- **SSN**: coperto nei centri specializzati [10].
- **Privato in Italia**: da **2.000 a 4.000 euro**.

## Body contouring

### Le procedure disponibili

Il body contouring comprende un insieme di procedure estetiche che rimodellano la silhouette corporea. Non sono specifiche per le persone trans, ma vengono utilizzate per accentuare la distribuzione femminile del grasso corporeo, talvolta come complemento alla terapia ormonale.

**Liposuzione**
Rimozione del grasso in eccesso da aree che tendono a mantenere una distribuzione maschile nonostante la terapia ormonale, come l'addome e i fianchi laterali (le cosiddette "maniglie dell'amore").

**Lipofilling (fat grafting)**
Il grasso prelevato con la liposuzione viene purificato e reiniettato in aree dove si desidera più volume: tipicamente fianchi, glutei e cosce. Permette di accentuare le curve femminili senza protesi.

**BBL (Brazilian Butt Lift)**
Un lipofilling specifico per i glutei. Il grasso viene prelevato da addome, fianchi o cosce e reiniettato nei glutei per ottenere una forma più tondeggiante. È una procedura che comporta rischi specifici significativi, tra cui embolia grassa, e va eseguita solo da chirurghi esperti.

### Recupero

- Liposuzione e lipofilling: **2-4 settimane** di recupero, con uso di guaine compressive per 4-6 settimane
- BBL: **3-6 settimane** di recupero, con il divieto di sedersi direttamente sui glutei per le prime 2-3 settimane

### Costi

- **SSN**: non coperto.
- **Privato in Italia**: liposuzione da **2.000 a 5.000 euro** per area; lipofilling da **3.000 a 6.000 euro**; BBL da **5.000 a 10.000 euro**.

## Epilazione definitiva

### Perché è rilevante

L'epilazione definitiva ricopre un doppio ruolo nel percorso delle donne trans:

**Pre-chirurgica**: prima della vaginoplastica è necessaria l'**epilazione dell'area genitale**, in particolare della pelle del pene e dello scroto che verrà utilizzata per creare il canale vaginale e la vulva [13]. Se questa pelle non viene epilata, i follicoli piliferi rimangono attivi e possono causare crescita di peli all'interno della neovagina, con rischi di infezioni e complicanze. L'epilazione pre-chirurgica richiede tipicamente **6-12 mesi** di trattamenti prima dell'intervento.

**Estetica**: la riduzione dei peli del viso e del corpo è uno degli interventi più richiesti dalle donne trans. La terapia estrogenica riduce la peluria corporea ma non elimina la barba, che richiede trattamenti specifici.

### Le tecniche

**Epilazione laser**
Il laser agisce sulla melanina del pelo, distruggendo il follicolo con impulsi di luce. È più efficace su peli scuri e pelle chiara. Richiede **6-12 sedute** distanziate di 4-6 settimane. Non è efficace su peli chiari, bianchi o rossi.

Tecnologie principali: laser Alessandrite (pelli chiare), laser Nd:YAG (pelli scure), laser a diodo (versatile).

**Elettrolisi (elettrodepilazione)**
L'elettrolisi distrugge il follicolo pilifero con un ago che emette corrente elettrica. È l'unico metodo **permanente al 100%** e funziona su tutti i colori di pelo e di pelle. È più lenta e dolorosa del laser: ogni pelo viene trattato singolarmente. Per il viso, un ciclo completo può richiedere **100-300 ore** di trattamento distribuite su 1-2 anni.

### Strategia combinata

La strategia più efficiente è combinare le due tecniche:
1. **Laser** per la riduzione iniziale (elimina il 70-80% dei peli scuri)
2. **Elettrolisi** per i peli residui, quelli chiari e quelli più resistenti

Per l'epilazione pre-vaginoplastica, uno studio sistematico del 2022 ha confrontato le due tecniche, confermando che entrambe sono efficaci e che la combinazione offre i risultati migliori [13].

### Costi

- **SSN**: generalmente non coperto, salvo l'epilazione pre-chirurgica in alcuni centri.
- **Laser viso (ciclo completo)**: da **1.500 a 3.000 euro** (6-12 sedute).
- **Elettrolisi viso (ciclo completo)**: da **3.000 a 8.000 euro** (100-300 ore).
- **Laser area genitale (pre-chirurgica)**: da **800 a 2.000 euro**.

## La sequenza degli interventi

Non esiste un ordine obbligatorio per gli interventi di affermazione di genere. La sequenza dipende dalle priorità individuali, dalla disponibilità dei centri, dalle condizioni cliniche e dalle risorse economiche. Tuttavia, esistono considerazioni pratiche e cliniche che rendono alcune sequenze più comuni di altre.

### Una sequenza frequente

1. **Terapia ormonale** (prerequisito per molti interventi): inizio degli estrogeni e degli antiandrogeni. I [cambiamenti fisici](/wiki/corpo-cambia-dopo-ormoni) iniziano dopo 1-3 mesi e si sviluppano nell'arco di 2-5 anni.

2. **Epilazione** (se si prevede la vaginoplastica): va iniziata con ampio anticipo, perché richiede 6-12 mesi di trattamenti [13].

3. **Condrolaringoplastica e/o FFS**: interventi che possono essere eseguiti anche nelle fasi iniziali della transizione, senza necessità di terapia ormonale pregressa. Per chi desidera la FFS, è talvolta preferibile operarsi dopo almeno 12-18 mesi di terapia ormonale, perché gli estrogeni modificano la distribuzione del grasso facciale e dei tessuti molli, influenzando la pianificazione chirurgica.

4. **Mammoplastica additiva**: dopo almeno 12 mesi di terapia ormonale [1]. Alcune pazienti attendono 2-3 anni per massimizzare lo sviluppo naturale.

5. **Orchiectomia**: può essere eseguita come procedura ponte durante l'attesa per la vaginoplastica, oppure come scelta definitiva [8].

6. **Vaginoplastica**: spesso l'ultimo intervento del percorso per ragioni di complessità e recupero. Richiede che l'epilazione sia completata e che la paziente possa dedicare 6-8 settimane al recupero iniziale.

7. **Chirurgia vocale**: in qualsiasi momento, spesso dopo aver tentato il voice training.

8. **Body contouring**: generalmente una delle ultime procedure, quando gli effetti della terapia ormonale sulla distribuzione del grasso si sono stabilizzati.

### Combinare gli interventi

Alcune procedure possono essere combinate nella stessa sessione chirurgica per ridurre il numero complessivo di anestesie e i tempi di recupero:

- **FFS combinata**: fronte, naso, mascella e mento in un'unica sessione (4-8 ore di intervento)
- **Condrolaringoplastica + mammoplastica**: a volte combinate
- **Orchiectomia + vaginoplastica**: l'orchiectomia è parte standard della vaginoplastica

La combinazione di interventi va sempre discussa con il chirurgo, valutando i rischi aggiuntivi di sessioni prolungate.

## Centri di riferimento

### In Italia

I centri pubblici di riferimento per la chirurgia di affermazione di genere in Italia sono coordinati dall'ONIG e includono [10]:

- **CIDIGEM - Torino** (AOU Città della Salute e della Scienza): centro storico con esperienza pluridecennale in vaginoplastica e mammoplastica
- **AOU Careggi - Firenze**: centro di riferimento per la Toscana
- **San Camillo-Forlanini - Roma**: centro multidisciplinare
- **Policlinico di Bari**: centro di riferimento per il sud Italia
- **AOU Federico II - Napoli**: centro specializzato
- **AOU di Bologna**: centro con equipe dedicate

Per la FFS e la chirurgia vocale, non esistendo una rete pubblica strutturata, ci si rivolge prevalentemente a chirurghi privati specializzati nelle principali città (Milano, Roma, Torino).

### In Europa

- **Ghent University Hospital (Belgio)**: centro di riferimento europeo per vaginoplastica e FFS
- **VU University Medical Center - Amsterdam (Paesi Bassi)**: tra i centri con la casistica più ampia al mondo
- **Team Facial - Marbella (Spagna)**: specializzato in FFS
- **Yeson Voice Center - Seoul (Corea del Sud)**: centro di riferimento mondiale per la chirurgia vocale
- **Kamol Cosmetic Hospital - Bangkok (Thailandia)**: centro ad alto volume per vaginoplastica e FFS

La scelta tra operarsi in Italia o all'estero dipende da molti fattori: copertura SSN, liste di attesa, esperienza del chirurgo, costi complessivi (inclusi viaggio e soggiorno) e la possibilità di follow-up nel post-operatorio.

## Pianificazione e decisioni

### Non esiste un percorso uguale per tutte

Ogni donna trans ha esigenze diverse. Alcune sperimentano disforia intensa legata ai genitali e considerano la vaginoplastica prioritaria. Altre avvertono la disforia principalmente sul viso e preferiscono la FFS. Altre ancora non desiderano alcun intervento chirurgico e vivono serenamente con la sola terapia ormonale o anche senza di essa.

Non esiste una gerarchia di interventi e non esiste un pacchetto standard. La chirurgia è uno strumento, non un obbligo.

### Fattori da considerare

- **Priorità personali**: quale fonte di disforia è più invalidante nel quotidiano?
- **Risorse economiche**: gli interventi non coperti dal SSN rappresentano un investimento significativo
- **Tempi di attesa**: le liste SSN possono essere molto lunghe; è utile mettersi in lista il prima possibile, anche se l'intervento è previsto per il futuro
- **Pianificazione lavorativa**: ogni intervento richiede un periodo di assenza dal lavoro
- **Supporto sociale**: avere persone fidate durante il recupero fa una differenza significativa
- **Salute generale**: alcune condizioni mediche possono influenzare l'idoneità chirurgica o i tempi

### Il ruolo del team multidisciplinare

Il percorso chirurgico ideale coinvolge un team multidisciplinare: endocrinologo, psicologo o psicoterapeuta, chirurgo specializzato ed eventualmente logopedista [1]. Questo team non ha il compito di decidere al posto della persona ma di accompagnarla con competenza, fornendo informazioni accurate, gestendo le aspettative e garantendo la sicurezza del percorso.

## Soddisfazione e rimpianto

I dati sulla soddisfazione dopo chirurgia di affermazione di genere nelle donne trans sono tra i più solidi della letteratura chirurgica. Una meta-analisi del 2021 ha calcolato un tasso di rimpianto complessivo **inferiore all'1-2%**, tra i più bassi in tutta la chirurgia elettiva [11]. Uno studio JAMA del 2024 ha confermato che il **99,7%** delle persone trans che hanno avuto accesso alla chirurgia riporta soddisfazione con i risultati [14].

I rari casi di rimpianto sono quasi sempre legati a complicanze post-operatorie, risultati estetici non soddisfacenti o mancanza di supporto sociale, non all'identità di genere in sé [11].

Questi dati non significano che la chirurgia sia priva di rischi o di momenti difficili. Il post-operatorio può essere fisicamente ed emotivamente impegnativo. Ma a lungo termine, la stragrande maggioranza delle persone che sceglie questo percorso riporta un miglioramento significativo della qualità della vita, della salute mentale e del benessere complessivo.

## Il quadro complessivo

La chirurgia di affermazione di genere per le donne trans non è un evento singolo ma un percorso che può estendersi su mesi o anni, con interventi diversi in tempi diversi. È un percorso individuale: non esiste un modello unico e nessuna scelta è obbligata.

Il panorama italiano offre sia opzioni pubbliche (con copertura SSN ma liste di attesa lunghe) sia opzioni private (con costi variabili ma tempi più rapidi). La sentenza 143/2024 della Corte Costituzionale ha rimosso un ostacolo burocratico significativo, riconoscendo che la scelta chirurgica spetta alla persona e al suo team medico, non a un tribunale [9].

I dati scientifici sono chiari: quando la chirurgia è desiderata e viene eseguita in centri specializzati, i tassi di soddisfazione sono tra i più alti in tutta la chirurgia e il rimpianto è un'eccezione rara [11][14]. La decisione di operarsi, tuttavia, rimane profondamente personale. Questo articolo serve a garantire che chi si pone la domanda abbia accesso a informazioni complete, aggiornate e basate sulle evidenze.
