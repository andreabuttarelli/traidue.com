<script lang="ts">
	...
	const url = $derived(`https://www.traidue.com/regione/${data.regione.slug}`);
	...
	let pageSchema = $derived({
		...
		'@type': 'WebPage',
		name: data.isRich
			? `Risorse trans in ${data.regione.nome}: centri, associazioni, iter`
			: `Risorse trans in ${data.regione.nome}`,
		description: data.dettaglio?.metaDescription ?? `Informazioni sulle tematiche trans per le persone in ${data.regione.nome}.`,
		url,
		inLanguage: 'it',
		isPartOf: {
			'@type': 'WebSite',
		...
		data.dettaglio
			? data.dettaglio.centri_gender.map((c) => ({
					...
					address: {
						...
						addressRegion: data.regione.nome,
						addressCountry: 'IT'
					},
...
<SEO
	title={data.isRich
		? `Risorse trans in ${data.regione.nome}: centri, associazioni, iter`
		: `Risorse trans in ${data.regione.nome}`}
	description={data.dettaglio?.metaDescription ?? `Informazioni sulle tematiche trans per le persone in ${data.regione.nome}. Articoli scientifici, risorse e supporto.`}
...
{#if data.isRich && data.dettaglio}
	<!-- ============================================ -->
	<!-- TEMPLATE RICCO — Regioni con dati dettagliati -->
	<!-- ============================================ -->
			...
			<article class="min-w-0 flex-1">
				...
				<header class="mb-10 sm:mb-14">
					...
					<div class="flex flex-wrap items-center gap-4 text-sm text-muted">
						...
						<ShareButtons {url} text="Risorse trans in {data.regione.nome}" />
					</div>
				</header>

				<!-- 2. Centri di identita di genere -->
				...
				<section class="mb-12 sm:mb-16">
					<h2 class="text-xl sm:text-2xl font-heading font-semibold tracking-tight text-primary mb-4">
						Iter sanitario in {data.regione.nome}
					</h2>
				...
				<section class="mb-12 sm:mb-16">
					<h2 class="text-xl sm:text-2xl font-heading font-semibold tracking-tight text-primary mb-4">
						Storia queer in {data.regione.nome}
					</h2>
					<p class="text-base text-primary/80 leading-relaxed mb-8 max-w-3xl">
				...
				<!-- 9. Altre regioni -->
				{#if data.altreRegioni.length > 0}
					<section class="mb-12 sm:mb-16">
						<h2 class="text-xl sm:text-2xl font-heading font-semibold tracking-tight text-primary mb-4">
							Altre regioni con guide dettagliate
						</h2>
						<div class="flex flex-wrap gap-2">
							{#each data.altreRegioni as reg}
								<a
									href="/regione/{reg.slug}"
...
{:else}
	<!-- ============================================ -->
	<!-- TEMPLATE GENERICO — Regioni senza dati ricchi -->
	<!-- ============================================ -->
	...
	<section>
		<div class="w-full px-4 sm:px-6 lg:px-12 py-16 sm:py-24 lg:py-32 text-center flex flex-col items-center">
			...
			<p class="text-sm text-muted mt-6 px-4 py-2 rounded-full border border-border">
				Guida dettagliata per {data.regione.nome} in arrivo
			</p>
		</div>
	</section>
